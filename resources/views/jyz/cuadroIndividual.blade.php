...
@section('content')
    ...
    <nav id="package-nav" class="w-full bg-white px-6 py-4 shadow-md sticky top-16 z-50 rounded-lg">
        ...
        <!-- Título de la página -->
        <h1 class="text-2xl md:text-3xl font-bold text-center mt-6" style="color: #A69177;">MODELOS DE CUADRO INDIVIDUAL</h1>
    </nav>
    ...
    <div class="mt-24">
        ...
        <form class="bg-custom2 max-w-5xl mx-auto p-8 bg-white rounded-xl shadow-xl space-y-6 mt-12"
            action="{{ route('agregarColores_individual') }}" method="POST" enctype="multipart/form-data">
            @csrf
...
<script>
    ...
    const maxFileSize = 10 * 1024 * 1024; // 10MB límite de tamaño de archivo
    ...
    ['dragenter', 'dragover'].forEach(eventName => {
        dropAreaColor.addEventListener(eventName, () => dropAreaColor.classList.add('border-indigo-600'), false);
    });

    ['dragleave', 'drop'].forEach(eventName => {
        dropAreaColor.addEventListener(eventName, () => dropAreaColor.classList.remove('border-indigo-600'), false);
    });
    ...
    function handleFileSelection(files) {
            ...
            if (files[0].type.startsWith('image/')) {
                if (files[0].size > maxFileSize) {
                    Swal.fire({
                        ...
                        text: 'El archivo supera el tamaño máximo permitido de 10MB. Por favor, reduzca el tamaño del archivo.',
                        timer: 5000,
                        timerProgressBar: true
                    });
                    fileInfoColor.textContent = 'Archivo no válido: demasiado grande.';
        ...
        <form class="bg-custom1 max-w-5xl mx-auto p-8 rounded-xl shadow-xl space-y-6 mt-12"
            action="{{ route('agregarModelos_individual') }}" method="POST" enctype="multipart/form-data">
            @csrf
        ...
        <form class="bg-custom4 bg-gray-50 max-w-5xl mx-auto p-8 bg-white rounded-xl shadow-xl space-y-6 mt-12"
            action="{{ route('agregarModelosCuadrosGrupo_individual') }}" method="POST" enctype="multipart/form-data">
            @csrf
            ...
            function eliminarModelo(id) {
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        ...
                        form.method = 'POST';
                        form.action = '{{ route('eliminarModelo_individual', ['id' => '__id__']) }}'.replace('__id__', id);
            ...
            function eliminarModeloCuadrosGrupo(id) {
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        var form = document.createElement('form');
                        form.method = 'POST';
                        form.action = '{{ route('eliminarModeloCuadrosGrupo_individual', ['id' => '__id__']) }}'.replace('__id__',
                            id);
                        form.style.display = 'none';
            ...
            function eliminarColor(id) {
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        var form = document.createElement('form');
                        form.method = 'POST';
                        form.action = '{{ route('eliminarColor_individual', ['id' => '__id__']) }}'.replace('__id__', id);
                        form.style.display = 'none';
