                ...
                <div class="mb-5">
                    ...
                    <div class="flex items-center">
                        <div class="grid grid-cols-4 gap-2 flex-grow">
                            <input type="password" name="password1" inputmode="numeric" maxlength="1" pattern="\d*"
                                ...
                                required>
                            <input type="password" name="password2" inputmode="numeric" maxlength="1" pattern="\d*"
                                class="password-input shadow-md appearance-none border border-[#59554D] rounded-lg py-3 px-4 text-[#A69177] text-center leading-tight focus:outline-none focus:ring-2 focus:ring-[#F2D0A7] focus:border-transparent transition duration-500"
                                required>
                            <input type="password" name="password3" inputmode="numeric" maxlength="1" pattern="\d*"
                                class="password-input shadow-md appearance-none border border-[#59554D] rounded-lg py-3 px-4 text-[#A69177] text-center leading-tight focus:outline-none focus:ring-2 focus:ring-[#F2D0A7] focus:border-transparent transition duration-500"
                                required>
                            <input type="password" name="password4" inputmode="numeric" maxlength="1" pattern="\d*"
                                class="password-input shadow-md appearance-none border border-[#59554D] rounded-lg py-3 px-4 text-[#A69177] text-center leading-tight focus:outline-none focus:ring-2 focus:ring-[#F2D0A7] focus:border-transparent transition duration-500"
                                required>
                        </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            form.addEventListener('submit', async (e) => {
                ...
                const password4 = document.querySelector('input[name="password4"]').value;

                // Combinar los valores de los campos de contraseña
                const combinedPassword = password1 + password2 + password3 + password4;

                // Establecer la contraseña combinada en el campo oculto
                document.getElementById('password').value = combinedPassword;

                const formData = new FormData(form);

                // Convertir FormData a un objeto para ver los datos en consola
                ...
                try {
                    ...
                    console.log('Response Text:', text); // Muestra la respuesta en la consola

                    // Intenta convertirla en JSON si crees que es un JSON válido
                    try {
                        const data = JSON.parse(text);
                        data.success ? window.location.href = data.redirect : handleErrors(data.message,
                            data.errorType, formData);
                    } catch (jsonError) {
                    ...
                    Swal.fire('Error',
                        'Hubo un error al procesar su solicitud. Por favor, intente nuevamente.',
                        'error');
                }
            });
            ...
            document.getElementById('telefono').addEventListener('input', () => {
                ...
                const formatted = input.value.replace(/\D/g, '').substring(0, 10)
            ...
            passwordInputs.forEach((input, idx) => {
                input.addEventListener('input', () => {
                    const fullPassword = Array.from(passwordInputs).map(i => i.value).join('');
                    document.getElementById('password').value = fullPassword;
                    if (input.value.length === input.maxLength && idx < passwordInputs.length - 1) {
            ...
            document.getElementById('togglePasswordIcon').addEventListener('click', () => {
                ...
                passwordInputs.forEach(input => input.type = isPassword ? 'text' : 'password');
...
<body
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        // Manejar el input de contraseña
        ...
        // Prevenir el comportamiento por defecto del formulario y manejar el envío
        document.getElementById('tuFormulario').addEventListener('submit', function(event) {
            event.preventDefault();
            formatPhoneNumber(); // Formatear el número de teléfono antes de enviar el formulario
            
            const formData = new FormData(this);
            // Aquí iría la llamada a la función que maneja el envío de datos
            // Por ejemplo, enviar los datos usando fetch o axios
            // fetch('/ruta/a/tu/api', {
            //     method: 'POST',
            //     body: formData
